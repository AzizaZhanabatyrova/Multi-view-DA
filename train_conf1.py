...
if args.target == 'isprs':
    IMG_MEAN_TARGET = np.array((84.96108839384782,  90.37637720260379, 93.43303655945203), dtype=np.float32) # ISPRS all BGR
else:
    IMG_MEAN_TARGET = np.array((101.41694189393208,  89.68194541655483, 77.79408426901315), dtype=np.float32) # crowdai all BGR


if args.source == 'airsim':
    weights1 = [1.0, 1.546678998390025, 1.7889711242828616, 2.535985082506768, 78.41646405001737] # for Airsim
    weights2 = [2.3549578963120164, 1.0, 1.9539765672618914, 2.5744951710722392, 10.197252624262202] # for Cityscapes
else:
    weights1 = [2.3549578963120164, 1.0, 1.9539765672618914, 2.5744951710722392, 10.197252624262202]
    weights2 = [1.0, 1.546678998390025, 1.7889711242828616, 2.535985082506768, 78.41646405001737] 
...
def main():
    ...
    if args.model == 'DeepLab':
        if args.training_option == 1:
            model = Res_Deeplab(num_classes=args.num_classes, num_layers = args.num_layers, dropout = args.dropout, after_layer = args.after_layer)
        ...
        for sub_i in range(args.iter_size):
            ...
            while True:
                try:
                    ...
                    loss_seg1 = loss_calc(pred1, labels, args.gpu, args.ignore_label, train_name, weights1)
